    ...
    <div :style="{width:tableWidth}" class='autoTbale'>
        <table class="table table-bordered" id='hl-tree-table'>
                ...
                <tr>
                    <th v-for="(column,index) in cloneColumns">
                        <label v-if="column.type === 'selection'">
                            <input type="checkbox" v-model="checks" @change="handleCheckAll">
                        </label>
                        <label v-else>
            ...
            <tbody>
                ...
                <tr v-for="(item,index) in initItems" :key="`${item.parent && item.parent.id}-${item.id}`" v-show="show(item)" :class="{'child-tr':item.parent,'active':(current==index)}" class="tableTr" @click="chooseTr(index,item)">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        <label v-if="column.type === 'selection'">
                            <input type="checkbox" v-model="item.isChecked" @change="handleCheckClick(item)">
                        </label>
                        <div v-if="column.type === 'action'">
                            <i-button style="margin-right:5px" :type="action.type" size="small" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</i-button>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type" :style="levelMarginStyle(item, snum)">
                            <span v-if='snum==iconRow()'>
                                <i v-html='item.spaceHtml'></i>
                                <i v-if="item.children&&item.children.length>0||!(item.leaf)" class="ivu-icon" :class="{'ivu-icon-arrow-right-b':!item.expanded,'ivu-icon-arrow-down-b':item.expanded }" style="cursor:pointer;vertical-align: middle;"></i>
...
export default {
    ...
    methods: {
        chooseTr(index,item) {
         this.current = index,
         this.$emit('on-tr-click',index,item)
        },
        levelMarginStyle(item, snum) {
            return snum === this.iconRow() ? `margin-left: ${(item.level - 1) * 10}px` : '';
        },
        // 有无多选框折叠位置优化
        iconRow() {
            ...
            if (!item.leaf) {
                if (item.expanded) {
                    ...
                    if (item.load) {
                        ...
                        if (item.dictCode) params.dictCode = item.dictCode
                        this.loadData(params, (data) => {
                            if (data.length > 0) {
                                ...
                                item.children.forEach((child, childIndex) => {
                                    ...
                                    if (this.initItems[index].isChecked) {
                                        this.$set(this.initItems[index + childIndex + 1], 'isChecked', true);
                                        this.checkGroup.push(child.id);
                                    }
                                })
                            }
                        });
        ...
        handleCheckClick(data, parentChecked){
            // data.isChecked = parentChecked !== undefined ? parentChecked : !data.isChecked;
            if( parentChecked !== undefined ){
                data.isChecked = parentChecked;
            }
            if(data.isChecked){
                this.checkGroup.push(data);
            }else {
                this.checkGroup.some((val, idx) => {
                    if(val.id == data.id){
                        this.checkGroup.splice(idx,1)
                    } 
                })
            }
            ...
            if(arr){
                for (let i=0; i<arr.length; i++){
                    this.handleCheckClick(arr[i], data.isChecked)
                }
            }
        },
        //checkbox 全选 选择事件
        handleCheckAll() {
            ...
            let allItems = this.All(this.items);
            if (this.checks) {
                this.checkGroup = this.getArray(this.checkGroup.concat(allItems))
            } else {
                this.checkGroup = []
            }
            this.initData(this.deepCopy(this.items), 1, null);
        ...
        checkAllGroupChange(data) {
            let hasAll = this.initItems.every(item => data.includes(item.id));
            if (this.dataLength > 0 && hasAll) {
                this.checks = true;
            } else {
                this.checks = false;
            }
